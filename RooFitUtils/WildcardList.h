...
public:
  ...
  WildcardList(const TString &match, MatchMode mode = kWildcard,
               const char *delim = " \t\n\r\f") {
    AddMatches(match, mode, delim);
  }

  WildcardList(const char *const matches[], size_t nmatch,
               MatchMode mode = kWildcard) {
    AddMatches(matches, nmatch, mode);
  }

  Int_t AddMatches(const TString &match, MatchMode mode = kWildcard,
                   const char *delim = " \t\n\r\f") {
    ...
    for (i = 0; i < nmatch; i++) {
      ...
      _exact[n + i] = s;
      _regexp[n + i] =
          (mode == kExact) ? 0 : new TRegexp(*s, mode == kWildcard);
    }
    delete a;
    return nmatch;
  }

  void AddMatches(const char *const matches[], size_t nmatch,
                  MatchMode mode = kWildcard) {
    size_t n = _exact.size();
    _exact.resize(n + nmatch);
    _regexp.resize(n + nmatch);
  ...
  void Add(const char *match, MatchMode mode = kWildcard) {
    ...
    _regexp.push_back((mode == kExact) ? 0
                                       : new TRegexp(*s, mode == kWildcard));
  }

  Int_t NumTerms() const { return _exact.size(); }

  const TString *Match(const TString &str, Int_t ind = -1) const {
    // Matches the string against any of the saved patterns, or just one if ind
    // is specified.
  ...
  Int_t MatchInd(const TString &str) const {
    ...
    for (size_t i = 0, n = _exact.size(); i < n; i++) {
      if (_regexp[i]) {
        if (str.Contains(*(const TRegexp *)_regexp[i]))
          return i;
      } else {
        if (*(const TString *)_exact[i] == str)
          return i;
      }
    }
    return -1;
  ...
  const TString *Pattern(Int_t i) const {
    ...
    return (i >= 0 && size_t(i) < _exact.size()) ? (const TString *)_exact[i]
                                                 : 0;
  }

  ~WildcardList() {
    size_t i, n;
    for (i = 0, n = _exact.size(); i < n; i++)
      delete (const TString *)_exact[i];
    for (i = 0, n = _regexp.size(); i < n; i++)
      delete (const TRegexp *)_regexp[i];
  }
