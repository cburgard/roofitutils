...
public:
  ...
  RooCustomizerEnhanced() {
    _masterLeafListIter = 0;
    _masterBranchListIter = 0;
  } // dummy null constructor so can load from ROOT prompt
  RooCustomizerEnhanced(const RooAbsArg &pdf, const char *name,
                        const char *wild = 0);
  ...
  virtual void printMultiline(std::ostream &os, Int_t content,
...
protected:
  ...
  ClassDefOverride(RooCustomizerEnhanced,
                   0) // Editing tool for RooAbsArg composite object expressions
};
}
