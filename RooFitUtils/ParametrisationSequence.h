...
public:
  ...
  ParametrisationSequence(const std::string &SequenceName = "",
                          const std::string &ParametersOfInterest = "");
  ~ParametrisationSequence();

  // Accessors
  void SetSequence(const std::list<ParametrisationScheme> &Sequence) {
    fSequence = Sequence;
  }
  ...
  void AppendParametersOfInterest(std::string ParametersOfInterest) {
    fParametersOfInterest += "," + ParametersOfInterest;
  }
