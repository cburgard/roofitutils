...
// Constructor
RooFitUtils::Channel::Channel( const std::string& ChannelName, RooAbsPdf* Pdf, RooAbsData* Data, const std::string& ParentName )
  :
...
{
  coutP(InputArguments) << "Channel::Channel(" << fName <<") new Channel from " << fParentMeasurement << " with PDF " << fPdf->GetName() << " and data " << fData->GetName() << "." << std::endl;
  ...
  coutP(ObjectHandling) << "Channel::Channel(" << fName << ") making workspace " << tmpWSName << " for using the workspace factory." << std::endl;
...
{
  coutI(InputArguments) << "Channel::~Channel(" << fName << ") cleaning up" << std::endl;
  // TODO
}
...
void RooFitUtils::Channel::RegulariseChannel()
{
  ...
  coutI(ObjectHandling) << "Channel::RegulariseChannel(" << fName << ") importing Pdf " << fPdf->GetName() << " in workspace " << fWorkSpace->GetName() << std::endl;
  fWorkSpace->import(*fPdf, RooFit::RenameAllNodes(fParentMeasurement.c_str()), RooFit::RenameAllVariables(fParentMeasurement.c_str()), RooFit::Silence());
  ...
  std::string tmpAllProducts = std::string(fPdf->GetName()) + "_" + fParentMeasurement;
  std::string newPdfName = std::string(fPdf->GetName()) + "_" + fParentMeasurement + "Corr";
  for (auto it = thisCorrelationFactors.begin(); it != thisCorrelationFactors.end(); ++it) {
  ...
  coutI(ObjectHandling) << "Channel::RegulariseChannel(" << fName << ") getting Pdf " << newPdfName << " from workspace" << fWorkSpace->GetName() << std::endl;
  fPdf = (RooAbsPdf*)fWorkSpace->pdf(newPdfName.c_str());

  // Regularise the Pdf
  const RooArgSet* obsSet = fData->get();

  coutI(ObjectHandling) << "Channel::RegulariseChannel(" << fName << ") started writing local editing command for PDF " << fPdf->GetName() << std::endl;
  ...
  for (auto itr = thisRenamingMap.begin(); itr != thisRenamingMap.end(); ++itr) {
    ...
    if (!fWorkSpace->var(newObsName.c_str()) && !fWorkSpace->obj(newObsName.c_str())) {
      coutE(ObjectHandling) << "Channel::RegulariseChannel(" << fName << ") variable " << newObsName  << " was not imported into workspace " << fWorkSpace->GetName() << ". Skipping it." << std::endl;
      continue;
    }
    ...
    std::string renamedVar = oldObsName + "_" + fParentMeasurement;
    if (!fWorkSpace->obj(renamedVar.c_str())) {
      coutE(ObjectHandling) << "Channel::RegulariseChannel(" << fName << ") object " << renamedVar << " was not imported into workspace " << fWorkSpace->GetName() << ". Skipping it." << std::endl;
      continue;
    }

    // Don't edit if the Pdf doesn't use the variable.
    if (!fPdf->dependsOn(*(RooAbsArg*)fWorkSpace->obj(renamedVar.c_str())))  {
      coutW(ObjectHandling) << "Channel::RegulariseChannel(" << fName << ") skipping variable " << renamedVar << std::endl;
      continue;
    }
    ...
    if (newConstraintType == "Gaussian" || newConstraintType == "Poisson") {
      ...
      coutE(ObjectHandling) << "Channel::RegulariseChannel(" << fName << ") did not recognize constraint type " << newConstraintType << ". Skipping." << std::endl;
      continue;
    }
    nrEdits++;
  }
  ...
  while ((nextNui = (RooRealVar*)nuiItr->Next())) {
    ...
    coutI(ObjectHandling) << "Channel::RegulariseChannel(" << fName << ") trying to get nuisance parameter " << argName.str() << std::endl;
    RooRealVar* nextNui_store = nextNui;
    nextNui = (RooRealVar*)fWorkSpace->obj(argName.str().c_str());

    if (!nextNui) {
      if (fPdf->dependsOn(*nextNui_store)) {
        coutF(ObjectHandling) << "Channel::RegulariseChannel(" << fName << ") couldn't find nuisance parameter " << argName.str() << " in workspace " << fWorkSpace->GetName() << " for channel " << fName << " from measurement " << fParentMeasurement << std::endl;
        exit(-1);
      } else {
        coutW(ObjectHandling) << "Channel::RegulariseChannel(" << fName << ") nuisance parameter " << argName.str() <<" in list of parameters, but Pdf " << fPdf->GetName() << "from channel " << fName << " has no dependence. Ignoring it." << std::endl;
        continue;
      }
    }

    if (nextNui->isFundamental()) {
      coutI(ObjectHandling) << "Channel::RegulariseChannel(" << fName <<  ") adding nuisance parameter: " << nextNui->GetName() << " from channel " << fName << " from measurement " << fParentMeasurement << std::endl;
  ...
  while ((nextObs = (RooRealVar*)obsSetItr->Next())) {
    RooRealVar* fromComb = (RooRealVar*)fWorkSpace->var((std::string(nextObs->GetName()) + "_" + fParentMeasurement).c_str());
    // RooAbsArg* fromComb = (RooAbsArg*)fWorkSpace->var((string(nextObs->GetName())).c_str());
    if (!fromComb) {
      coutW(ObjectHandling) << "Channel::RegulariseChannel(" << fName << ") variable " << std::string(nextObs->GetName()) << " is not in workspace " << fWorkSpace->GetName() << "." << std::endl;
      continue;
    }
  ...
  editStr << ")";
  coutP(ObjectHandling) << "Channel::RegulariseChannel(" << fName << ") editing command so far " << editStr.str() << std::endl;
  ...
  if (nrEdits != 0) {
    ...
    if (!fPdf) {
      coutF(ObjectHandling) << "Channel::RegulariseChannel(" << fName << ") something went wrong when editing the Pdf for channel " << fName << "." << std::endl;
      exit(-1);
    }
  }

  // taking care of the global observables
  coutP(ObjectHandling) << "Channel::RegulariseChannel(" << fName << ") adding global observables for model " << fParentMeasurement << std::endl;
  ...
  while ((nextGlob = (RooRealVar*)obsItr->Next())) {
    ...
    if (!nextGlob) {
      if (fPdf->dependsOn(*nextGlob_store)) {
        coutF(ObjectHandling) << "Channel::RegulariseChannel(" << fName << ") couldn't find global observable: " << argName.str() << " in workspace for model " << fParentMeasurement << std::endl;
        exit(-1);
      } else {
        coutW(ObjectHandling) << "Channel::RegulariseChannel(" << fName << ") global observable in list of global observables, but pdf has no dependence. Ignoring it." << std::endl;
        continue;
      }
    }

    coutI(ObjectHandling) << "Channel::RegulariseChannel(" << fName << ") adding global observable " << nextGlob->GetName() << " from model " << fParentMeasurement << std::endl;
