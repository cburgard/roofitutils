  ...
  fDataName( DataName )
{
  ...
  coutP(InputArguments) << "AbsMeasurement::AbsMeasurement(" << fName <<") created" << std::endl;
}
...
void AbsMeasurement::enablePruning( const std::string& poi, const std::string& filter, const std::string& weight, const std::string& threshold, int additionalDigit )
{
  ...
  std::cout << "\n****************************************************************************" << std::endl;
  std::cout << "* WARNING: Parameters will be pruned automatically based on the change of  *" << std::endl;
  std::cout << "*          Hesse uncertainty on the specified parameters of interest       *" << std::endl;
  std::cout << "*          respecting the specified thresholds.                            *" << std::endl;
  std::cout << "*                                                                          *" << std::endl;
  std::cout << "*          It is recommended to verify the list of pruned parameters!      *" << std::endl;
  std::cout << "****************************************************************************\n" << std::endl;
...
void RooFitUtils::AbsMeasurement::SetPrunedNuisanceParameters( std::string parameters )
{
  ...
  for (unsigned int itrPar = 0; itrPar < numParameters; ++itrPar) {
    TString thisPar = ((TObjString*)allParametersArray->At(itrPar))->GetString();
...
void  RooFitUtils::AbsMeasurement::PruneNuisanceParameters()
{
  ...
  for (RooLinkedListIter it = fModelConfig->GetParametersOfInterest()->iterator(); RooRealVar* v = dynamic_cast<RooRealVar*>(it.Next());) {
    v->setConstant(1);
  }
  ...
  fPrunedNuisanceParameters = PruneNuisanceParameters(hesse, fitresult, fPruningPoi, fPruningFilter, fPruningWeight, fPruningThreshold, fPruningAdditionalDigit, fPrunedNuisanceParameters);
...
// OR of the individual rankings.
  ...
  if (numPoi != numWeight && numWeight != 1) {
    coutE(ObjectHandling) << "AbsMeasurement::PruneNuisanceParameters(" << fName << ") Number of POIs does not match number of specified weights. No additional pruning." << std::endl;
    return prePrunedParameters;
  }

  // Check if number of POIs and thresholds match
  if (numPoi != numThreshold && numThreshold !=1) {
    coutE(ObjectHandling) << "AbsMeasurement::PruneNuisanceParameters(" << fName << ") Number of POIs does not match number of specified thresholds. No additional pruning." << std::endl;
    return prePrunedParameters;
  }
  ...
  for (unsigned int itrPoi = 0; itrPoi < numPoi; ++itrPoi) {
    ...
    coutI(ObjectHandling) << "AbsMeasurement::PruneNuisanceParameters(" << fName << ") Parsed POI " << thisPoi.Data()
                          << ", assigning weight " << tmpWeight
                          << " and threshold " << thisThreshold.Data() << std::endl;
  }

  if (numFilter > 0) {
    coutI(ObjectHandling) << "AbsMeasurement::PruneNuisanceParameters(" << fName << ") Nuisance parameters will be pruned in the following order:";
    ...
    coutI(ObjectHandling) << "AbsMeasurement::PruneNuisanceParameters(" << fName << ") Include all parameters in ranking" << std::endl;
  ...
  // Normalize weights
  for (auto itrWeight = WeightList.begin(); itrWeight != WeightList.end(); ++itrWeight) {
  ...
  for (std::vector<TString>::const_iterator itr = PoiList.begin(), end = PoiList.end(); itr != end; ++itr) {
    ...
    std::pair<double, double> tmpVals = PDGrounding(thisBestFit, thisInitErr, additionalDigit);
    ...
    coutI(ObjectHandling) << "AbsMeasurement::PruneNuisanceParameters(" << fName << ") Harvest parameter (" << thisIndex << "): " << *itr
                          << " with Hesse error " << tmpVals.first << " (" << thisBestFit
                          << ") +/- " << tmpVals.second << " (" << thisInitErr << ")" << std::endl;
  }
  ...
  for (std::vector<TString>::const_iterator itr = PoiList.begin(), end = PoiList.end(); itr != end; ++itr) {
    for (std::vector<TString>::const_iterator jtr = PoiList.begin(), end = PoiList.end(); jtr != end; ++jtr) {
  ...
  coutI(ObjectHandling) << "AbsMeasurement::PruneNuisanceParameters(" << fName << ") Initial averaged POI with propagated uncertainties: "
  ...
  for (std::vector<TString>::const_iterator itrFilter = FilterList.begin(), end = FilterList.end(); itrFilter != end; ++itrFilter) {
    ...
    while ((unsigned int)order.size() < allParams - (numPoi + 1)) {
      ...
      std::map< std::string, std::set< std::pair< double, std::string > > > part_uncerts;
      ...
      for (RooLinkedListIter it = pars.iterator(); RooRealVar* v = dynamic_cast<RooRealVar*>(it.Next());) {
        ...
        for (std::vector<TString>::const_iterator i = PoiList.begin(), end = PoiList.end(); i != end; ++i) {
          for (std::vector<TString>::const_iterator j = PoiList.begin(), end = PoiList.end(); j != end; ++j) {
      ...
      std::cout << std::endl;
      coutI(ObjectHandling) << "AbsMeasurement::PruneNuisanceParameters(" << fName << ") Ranking of remaining parameters based on combined inclusive POI" << std::endl;
      ...
      for (std::set<std::pair<double, std::string> >::reverse_iterator rankitr = uncerts.rbegin(); rankitr != uncerts.rend(); ++rankitr) {
        par2rem = rankitr->second;
        par2rem_err = rankitr->first;
        bool passThreshold = true;
        int tmpIndex = 0;

        coutI(ObjectHandling) << "AbsMeasurement::PruneNuisanceParameters(" << fName << ") --> Testing " << par2rem << std::endl;;

        // Loop over the individual POIs to test effect of removing proposed parameter on each of them
        for(auto iterator = part_uncerts.begin(); iterator != part_uncerts.end(); ++iterator) {
          std::string thisPoi = iterator->first;

          coutI(ObjectHandling) << "AbsMeasurement::PruneNuisanceParameters(" << fName << ")  | " << thisPoi;
          ...
          for (std::set<std::pair<double, std::string> >::reverse_iterator subitr = thisRanking.rbegin(); subitr != thisRanking.rend(); ++subitr) {
            ...
            if (varName == par2rem) {
              ...
              if (thisThreshold != "auto") {
                ...
                std::pair<double, double> tmpVals = PDGrounding(bestFit[thisPoi], thisUncert, additionalDigit);
                ...
                std::cout << ": absolute change " << initErrSig[thisPoi] << " --> " << tmpVals.second;
                ...
                if (tmpVals.second < initErrSig[thisPoi]) {
                  if (additionalDigit > 0) {
                    if (!AlmostEqualUlpsAndAbs(lastInit, lastRed, ThresholdCutList[thisPoi], 4)) {
                      std::cout << " reduced too much: " << lastInit << " - " << lastRed << " > " << ThresholdCutList[thisPoi];
                      passThreshold = false;
                    } else {
                      std::cout << " tolerated: " << lastInit << " - " << lastRed << " < " << ThresholdCutList[thisPoi];
                    }
                  } else {
                    passThreshold = false;
                  }
        ...
        if (!passThreshold) {
          coutI(ObjectHandling) << "AbsMeasurement::PruneNuisanceParameters(" << fName << ")  └-> Vetoed parameter because uncertainty on one POI reduced too much!" << std::endl;
        } else {
          coutI(ObjectHandling) << "AbsMeasurement::PruneNuisanceParameters(" << fName << ")  └-> Removing parameter!" << std::endl;
          foundPar2Rem = true;
          break;
        }
      }
      ...
      if (!foundPar2Rem) {
        coutI(ObjectHandling) << "AbsMeasurement::PruneNuisanceParameters(" << fName << ")  └-> No more parameters found to prune!" << std::endl;
        break;
      }
  ...
  coutI(ObjectHandling) << "AbsMeasurement::PruneNuisanceParameters(" << fName << ") Ranking of all non-filtered parameters based on combined inclusive POI" << std::endl;
...
void  RooFitUtils::AbsMeasurement::PrintRanking( std::set< std::pair< double, std::string > > uncerts, double initTotalError )
{
  ...
  for (std::set< std::pair< double, std::string> >::reverse_iterator itr = uncerts.rbegin(); itr != uncerts.rend(); ++itr) {
    ...
    coutI(ObjectHandling) << "AbsMeasurement::PrintRanking(" << fName << ") "
                          << Form("%09f%% (%05f -> %05f): %s",
                                  delta*100, initTotalError, uncert, varName.c_str()) << std::endl;
  }
  std::cout << std::endl;
}
...
void TOwnedList::Clear(Option_t *option)
{
  if (!option || strcmp(option,"nodelete")!=0)
    for (TIter it(this); TObject* obj= it();) {
      // cout << "Delete "<<obj->ClassName()<<"::"<<obj->GetName()<<(obj->IsOnHeap()?"":" (not on heap)")<<std::endl;
      delete obj;
    }
                                               ...
                                               const char* weightVarName )
{
  ...
  for (TIter nextds= dataList; RooAbsData* datai= dynamic_cast<RooAbsData*>(nextds());) {
    ...
    coutI(ObjectHandling) << "AbsMeasurement::SetDatasetBinning(" << fName << ") "
                          << Form("Category %s%s dataset has %d/%g entries",
                                  datai->GetName(), dataType, datai->numEntries(), datai->sumEntries()) << std::endl;
    ...
    coutP(ObjectHandling) << "AbsMeasurement::SetDatasetBinning(" << fName << ") "
                          << Form("%s binning on%s dataset %s with %d/%g entries, PDF %s, variables",
                                  (rebin?"Prune":genBinOnly?"Generate":"Set"),
                                  dataType, datai->GetName(), datai->numEntries(), datai->sumEntries(), pdfi->GetName()) << std::endl;
    ...
    if (obs->getSize()!=1 && !rebin) {
      std::cerr << "PDF "<<pdfi->GetName()<<" has "<<obs->getSize()<<" observables - can't create binned dataset" <<std::endl;
      delete obs;
      continue;
    }
    ...
    if (rebin) {
      ...
      coutI(ObjectHandling) << "AbsMeasurement::SetDatasetBinning(" << fName << ") "
                            << Form("Observable %s bins (%d,%g,%g) contain %d/%g entries:",
                                    ...
                                    datai->numEntries(), datai->sumEntries()) << std::endl;
      for (int j=0, n=datai->numEntries(); j<n; j++) {
        const RooArgSet* row = datai->get(j);
        Double_t w= datai->weight();
        if (w == 0) continue;
        coutI(ObjectHandling) << "AbsMeasurement::SetDatasetBinning(" << fName << ") "
                              <<" "<<dynamic_cast<RooRealVar*>(row->find(*obsVar))->getVal()<<":"<<w << std::endl;
        dataiNew->add (*row, w);
      }
      std::cout << std::endl;
      ...
      coutI(ObjectHandling) << "AbsMeasurement::SetDatasetBinning(" << fName << ") "
                            << Form("Rebin dataset from %d/%g to %d/%g in %s", datai->numEntries(), datai->sumEntries(), dataiNew->numEntries(), dataiNew->sumEntries(), dataiNew->GetName()) << std::endl;
    } else {
      TString histName= Form ("%s_hist",   datai->GetName());
      TH1* hist= datai->createHistogram (histName, *obsVar);
      coutI(ObjectHandling) << "AbsMeasurement::SetDatasetBinning(" << fName << ") "
                            << Form("Created histogram %s(%d,%g,%g)",
                                    hist->GetName(), hist->GetNbinsX(), hist->GetXaxis()->GetXmin(), hist->GetXaxis()->GetXmax()) << std::endl;
      ...
      coutI(ObjectHandling) << "AbsMeasurement::SetDatasetBinning(" << fName << ") "
                            << Form("in dataset %s with %d/%g entries",
                                    dataiNew->GetName(), dataiNew->numEntries(), dataiNew->sumEntries()) << std::endl;
      delete hist;
    }
    delete obs;
  ...
  if (!catVar) {
    std::cerr << "Dataset "<<data->GetName()<<" does not have an index variable '"
         << cat.GetName() << "' - can't create binned dataset" << std::endl;
    return 0;
  }
  ...
  coutP(ObjectHandling) << "AbsMeasurement::SetDatasetBinning(" << fName << ") "
                        << Form("Replace dataset %s (%d/%g entries) with dataset %s (%d/%g entries)",
                                data->GetName(),    data->numEntries(),    data->sumEntries(),
                                newData->GetName(), newData->numEntries(), newData->sumEntries()) << std::endl;
