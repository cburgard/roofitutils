...
Copy workspace to another file, with renaming and edits, removing disconnected items.
...
Examples:
  ...
  root -b -q wsin.root editws.cc+'("wsout.root", "EDIT::OLDPDF(OLDPDF,relM=sum::(mH,-125)); oldvar=newvar,var*=Var*")'
...
See http://root.cern.ch/root/html/RooFactoryWSTool.html#RooFactoryWSTool:process for
...
replaces *s in NEWVAR. Note that all the rename specs are performed together (serially)
...
namespace {
  ...
  public:
    Rename(const char* p=0) {
      if (!p || !p[0]) return;
#ifdef __CINT__
#pragma optimize 0   // turn off CINT bytecode optimization, which doesn't work for the following (ROOT bug #75875 ?)
#endif
      for (;;) {
...
#pragma optimize 4   // restore default CINT optimization
    ...
    int rename (TString& name, bool verbose=true, bool canRemove=false, bool track=true) const {
      // Rename Note that substitutions occur serially, so can undo several edits
      int nmod=0;
      TString oldname= name;
      for (size_t i=0, nre=_re.size(); i<nre; i++) {
        if (!_re[i].Length()) {
          if (name != _exact[i]) continue;
          name= _sub[i];
        } else {
          if (!TPRegexp(_re[i]).Substitute(name,_sub[i])) continue;
        }
      ...
      if (verbose && nmod) {
        if (name.Length()) INFO("Rename %s -> %s",oldname.Data(),name.Data());
        else               INFO("Do not copy %s", oldname.Data());
      }
      return nmod;
    }

    TString renamed (const char* name, bool verbose=true, bool canRemove=false, bool track=true) const {
      TString n= name;
      rename (n, verbose, canRemove, track);
      return n;
    }

    static bool wild2subs (const TString& s, TString& exact, TString& re, TString& sub, TString& spec) {
      sub= "";
      Int_t ie= s.Index('=');
      if (ie==kNPOS || ie==0 || s.Index("=",ie+1)!=kNPOS || s.Index("$",ie+1)!=kNPOS) {
      ...
      re= "^"+re+"$";
      Int_t n1= TPRegexp("(?:\\*+|\\?+|\\[[^]]+\\])+").Substitute(re,"($&)","g");
      if (!n1) {
        re= "";
      ...
      if (gDebug>0) INFO("Replace s/%s/%s/g",re.Data(),sub.Data());
      return true;
    }
  };

  const char* Scan (const char* a, std::vector<std::string>& v, char sep=',', bool append=false)
  {
    if (!append) v.clear();
    if (!*a) return a;
    for (;; a++) {
  ...
  TObject* GetDataMember (const TObject* o, const char* name, const TClass* cls=0)
  {
    ...
    if (!dm) return 0;
    if (cls && strcmp(dm->GetTypeName(), cls->GetName()) != 0) return 0;
    if (dm->IsaPointer())
  ...
  std::string PrintArg (const RooAbsArg& a, Int_t level=2)
  {
    ...
    if (level>=3) { std::cout << " = "; a.printValue(os); }
    if (level>=4) { std::cout << " ";   a.printExtras(os); }
    return os.str();
  }

  RooAbsArg* import (RooWorkspace& ws, const RooAbsArg& a, const char* newname=0,
                     ...
                     const RooCmdArg& arg7=RooCmdArg(), const RooCmdArg& arg8=RooCmdArg())
  {
    ...
    RooMsgService::instance().setGlobalKillBelow (RooFit::PROGRESS);  // import(RooAbsData), which may be called internally, doesn't respect RooFit::Silence()
    ws.import (a, RooFit::Silence(), arg1, arg2, arg3, arg4, arg5, arg6, arg7, arg8);
    RooMsgService::instance().setGlobalKillBelow (msglevel);
    RooAbsArg* b= ws.arg(name);
    if (!b) WARN("Imported %s::%s not found",a.ClassName(),name.Data());
    return b;
  }

  RooAbsData* import (RooWorkspace& ws, RooAbsData& a, const char* newname=0,
                      const RooCmdArg& arg1=RooCmdArg(), const RooCmdArg& arg2=RooCmdArg(), const RooCmdArg& arg3=RooCmdArg(),
                      const RooCmdArg& arg4=RooCmdArg(), const RooCmdArg& arg5=RooCmdArg(), const RooCmdArg& arg6=RooCmdArg(),
  ...
  size_t ReadFile (const char* filename, std::vector<std::string>& v, bool strip=true, char comment='#', bool append=true)
  {
    if (!append) v.clear();
    ...
    while (f) {
      ...
      if (strip) {
        if (comment!='\0') {
          size_t ic= s.find('#');
          if (ic!=std::string::npos) s.erase(ic);
        }
        size_t i1= s.find_first_not_of(' ');
        if (i1==std::string::npos) continue;   // blank line
  ...
  RooArgSet copyset (RooWorkspace& w, const RooArgSet* in, const Rename& rename, const char* listtype="args", bool renameInput=false)
  {
    RooArgSet out;
    if (!in || in->getSize()<=0) return out;
    ...
    while ((a= it.next())) {
      ...
      if (!b) { WARN("%s::%s not copied to %s",a->ClassName(),a->GetName(),listtype); continue; }
      out.add(*b);
      // Should be OK to rename arg in the input set if it is our own snapshot
      if (renameInput && renamed) a->SetName(newname);
    }
    return out;
  }

  int importSnaphots (const RooWorkspace* w, RooWorkspace& wout, const Rename& rename)
  {
    const RooLinkedList* snaps= dynamic_cast<const RooLinkedList*>(GetDataMember (w, "_snapshots", RooLinkedList::Class()));
    if (!snaps) return 1;
    ...
    while ((o= it.Next())) {
      ...
      if (!newname.Length()) continue;   // rename to "" to remove
      RooArgSet* newsnap= dynamic_cast<RooArgSet*>(snap->snapshot());
      newsnap->setName(newname);
      RooArgSet orig= copyset (wout, newsnap, rename, Form("snapshot '%s' args",newname.Data()), true);
  ...
  RooCmdArg renameVariable (const RooAbsCollection* components, const Rename& rename, int* cont=0, std::vector<std::string>* newTitle=0)
  {
    if (!rename.size()) return RooCmdArg();
    RooFIter it= components->fwdIterator();
    RooAbsArg* a;
    TString from=",", to=",";
    bool docont=false;
    for (int ind=0; (a= it.next()); ind++) {
      if (cont && ind < *cont) continue;    // skip to where we left off
      TString oldname= a->GetName(), newname=oldname;
      if (!rename.rename(newname,false)) continue;
      if (!newname.Length() || components->find(newname) || to.Index(","+newname+",")!=kNPOS) {
        WARN("Cannot rename %s to %s - duplicate name", oldname.Data(),newname.Data());
        continue;
      }
      if (cont && ind>0 && (from.Length()+oldname.Length()-1 > RooFit_buflen || to.Length()+newname.Length()-1 > RooFit_buflen)) {
        *cont= ind;
        docont= true;
        break;
      }
      ...
      from += ",";
      to   += ",";
      if (newTitle && a->GetTitle() == oldname) newTitle->push_back(newname.Data());
    }
    if (cont && !docont) *cont=0;
    if (from.Length()<=2) return RooCmdArg();
    from= from(1,from.Length()-2);
    to=     to(1,  to.Length()-2);
    if (from.Length()>RooFit_buflen || to.Length()>RooFit_buflen)
      WARN( "RooFit::RenameVariable(from,to) string lengths (%d,%d) may be too long for RooWorkspace::import()",from.Length(),to.Length());
    return RooFit::RenameVariable(from,to);
  }

  Bool_t importPdf (RooWorkspace& w, RooAbsPdf* pdf, const Rename& rename, TString& newname, const char* pdftype="PDF")
  {
    if (!pdf) { newname=""; return false; }
    newname= pdf->GetName();
    RooCmdArg rentop;
    if (rename.rename(newname)) rentop= RooFit::RenameVariable(pdf->GetName(),newname);  // rename top-level pdf first
    if (w.pdf(newname)) {
      WARN("%s %s::%s already in workspace '%s'",pdftype,pdf->ClassName(),newname.Data(),w.GetName());
      return false;
    }
    Bool_t err= false;
    RooAbsCollection* components= RooArgSet(*pdf).snapshot();  // easy (if not so quick) way of getting names of all dependents
    ...
    for (int itmp=0;; itmp++) {
      ...
      if (cont>0) {  // work round stupid limit on length of rename args
        ...
        INFO("Import %s %s::%s to temporary workspace '%s' after testing %d/%d components",pdftype,pdf->ClassName(),newname.Data(),tmpws->GetName(),cont,components->getSize());
        imppdf= import (*tmpws, *imppdf, newname, RooFit::RecycleConflictNodes(), rentop, ren);
        rentop= RooCmdArg();
        err= !imppdf;
        delete savews;
        savews= tmpws;
        ...
        INFO("Import %s %s::%s to workspace '%s'",pdftype,pdf->ClassName(),newname.Data(),w.GetName());
        err= !import (w, *imppdf, newname, RooFit::RecycleConflictNodes(), RooFit::Silence(), rentop, ren);
        break;
      }
    }
    delete savews;
    ...
    for (size_t i=0, n=newTitle.size(); i<n; i++) {
      ...
      if (a) a->SetTitle (a->GetName());
    }
    return err;
  }

  int importModel (RooWorkspace& w, const RooStats::ModelConfig& mc, const Rename& rename, const char* mcname=0)
  {
    int nerr= 0;
    ...
    if (!mcname) mcname= mc.GetName();  // otherwise may have been stored by an alias
    RooStats::ModelConfig mcout (rename.renamed(mcname), &w);
    if (newPdf     .Length())                  mcout.SetPdf                    (newPdf);
    if (newPriorPdf.Length())                  mcout.SetPriorPdf               (newPriorPdf);
    const RooAbsData* pdata;
    if ((pdata= mc.GetProtoData()))            mcout.SetProtoData              (pdata->GetName());
    const RooArgSet* set;
    if ((set= mc.GetParametersOfInterest()))   mcout.SetParametersOfInterest   (copyset (w, set, rename, "POIs"));
    if ((set= mc.GetNuisanceParameters()))     mcout.SetNuisanceParameters     (copyset (w, set, rename, "NPs"));
    if ((set= mc.GetGlobalObservables()))      mcout.SetGlobalObservables      (copyset (w, set, rename, "global observables"));
    if ((set= mc.GetObservables()))            mcout.SetObservables            (copyset (w, set, rename, "observables"));
    if ((set= mc.GetConditionalObservables())) mcout.SetConditionalObservables (copyset (w, set, rename, "conditional observables"));
    if ((set= mc.GetConstraintParameters()))   mcout.SetConstraintParameters   (copyset (w, set, rename, "constraints"));
  ...
  int importData (RooWorkspace& w, RooAbsData* data, const Rename& rename)
  {
    ...
    RooCmdArg opt;
    if (rename.rename(dname,true,true)) opt= RooFit::Rename(dname);
    if (!dname.Length()) return 0;
    RooCmdArg ren= renameVariable (data->get(), rename);
    INFO("Import %s::%s to workspace '%s'",data->ClassName(),dname.Data(),w.GetName());
...
int editws (const RooWorkspace* w, RooWorkspace& wout, const char* mcname, const char* dataname)
{
  // Copy workspace, all models (or just mcname), all datasets (or just dataname), all snapshots,
  ...
  delete RooFitUtils::renameSpec; RooFitUtils::renameSpec=0;
  TString wsname= wout.GetName();
  if (ren.rename(wsname)) wout.SetName(wsname);

  int nerr= 0;
  if (dataname && dataname[0]) {
    RooAbsData* data= w->data(dataname);
    if (data) nerr += importData (wout, data, ren);
  } else {
  ...
  if (mcname && mcname[0]) {
    RooStats::ModelConfig* mc= dynamic_cast<RooStats::ModelConfig*>(w->genobj(mcname));
    if (mc) nerr += importModel (wout, *mc, ren, mcname);
  } else {
    // Access _genObjects list directly (instead of via allGenericObjects()) to see alias names
    const RooLinkedList* objs= dynamic_cast<const RooLinkedList*>(GetDataMember (w, "_genObjects", RooLinkedList::Class()));
    ...
    while ((obj= ito.Next())) {
      ...
      if (obj->IsA()==RooTObjWrap::Class()) {
        ...
        if (wobj) obj= wobj->obj();
      }
      if (obj->IsA() == RooStats::ModelConfig::Class()) {
        RooStats::ModelConfig* mc= dynamic_cast<RooStats::ModelConfig*>(obj);
        if (mc) nerr += importModel (wout, *mc, ren, name);
      } else {
        ren.rename(name);
        INFO("Import %s::%s to workspace '%s'", obj->ClassName(), name.Data(), wout.GetName());
        if (name == obj->GetName())
  ...
  for (std::map<std::string,int>::const_iterator i=orig.begin(), e=orig.end(); i!=e; i++)
...
int editws (const std::vector<std::string>& lines,RooWorkspace*w,const char* mcname){
  int nerr=0;
  TObjArray allmc;
  if (mcname && mcname[0]) {
    TObject* obj= w->genobj(mcname);
    if (obj && obj->IsA() == RooStats::ModelConfig::Class()) allmc.Add(obj);
  } else {
  ...
  if (!allmc.GetEntries()) { WARN("No ModelConfig found in workspace '%s'",w->GetName()); nerr++; }
  for (size_t il=0, nl=lines.size(); il<nl; il++) {
    if (lines[il] == "NOCOPY") {
      nocopy++;
    } if (allmc.GetEntries()>0 && lines[il].find("OLDPDF") != std::string::npos) {
      TObjArray pdfNames(allmc.GetEntries());  // Use TNamed to map old->new PDF nnames with name->title
      pdfNames.SetOwner();
      Int_t i, n=allmc.GetEntries();
      for (i=0; i<n; i++) {
        RooStats::ModelConfig* mc= dynamic_cast<RooStats::ModelConfig*>(allmc.At(i));
        if (!mc) continue;
        RooAbsPdf* pdf= mc->GetPdf();
        if (pdf) pdfNames.AddAt (new TNamed(pdf->GetName(),""), i);
      }
      for (i=0; i<n; i++) {
        ...
        if (done == pdfName) {  // didn't do this pdf before
          ...
          RooFitUtils::currentMC= dynamic_cast<RooStats::ModelConfig*>(allmc.At(i));
          RooAbsArg* a= w->factory(es);
          RooFitUtils::currentMC= 0;
          if (!a) continue;
          std::cout << "-> " << PrintArg(*a) << std::endl;
          if (a->InheritsFrom (RooAbsPdf::Class())) newPdfName= a->GetName();
          else nerr++;
        } else
        ...
        if (newPdfName.Length()>0 && newPdfName != oldPdfName) {
          ...
          INFO("Set '%s' as PDF in model '%s'", newPdfName.Data(), mc->GetName());
        }
      }
    } else if (lines[il].find("=") != std::string::npos && lines[il].find_first_of("({})") == std::string::npos) {
      if (!RooFitUtils::renameSpec) RooFitUtils::renameSpec= new std::string;
      if (RooFitUtils::renameSpec->size()) *RooFitUtils::renameSpec += ",";
      *RooFitUtils::renameSpec += lines[il];
    } else {
      const char* e= lines[il].c_str();
      std::cout << e << std::endl;
      if (allmc.GetEntries()>0) RooFitUtils::currentMC= dynamic_cast<RooStats::ModelConfig*>(allmc.At(0));
      RooAbsArg* a= w->factory(e);
      RooFitUtils::currentMC= 0;
      if (a) { std::cout << "-> " << PrintArg(*a) << std::endl; }
    }
  }
  return nerr;
}
...
int editws (const char* edit, RooWorkspace* w, const char* mcname)
{
  ...
  // Any lines containing the string "OLDPDF" will be run with the PDF name from each ModelConfig.
  if (!edit || !edit[0]) return 0;
  int nerr=0;
  ...
  if (strpbrk("([{}]);=",edit)) {
    ...
    for (size_t jf=0, nf=files.size(); jf<nf; jf++) {
      if (!ReadFile (files[jf].c_str(), lines)) nerr++;
    }
  }
  return nerr + editws(lines,w,mcname);
}

int editws (RooWorkspace* w, TDirectory* fout, const char* edit, const char* mcname, const char* dataname)
{
  int nerr= 0;
  if (edit) nerr += editws (edit, w, mcname);
  if (nocopy) {
...
int editws (RooWorkspace* w, RooWorkspace* wout, const char* edit, const char* mcname, const char* dataname)
{
  int nerr= 0;
  if(!wout) return -1;
  if (edit) nerr += editws (edit, w, mcname);
...
int editws (RooWorkspace* w, RooWorkspace* wout, const std::vector<std::string>& edits, const char* mcname, const char* dataname)
{
  int nerr= 0;
  if(!wout) return -1;
  if (edits.size()>0) nerr += editws (edits, w, mcname);
  nerr += editws (w, *wout, mcname, dataname);
  return nerr;
}

int editws (RooWorkspace* w, const char* outfile, const char* edit, const char* mcname, const char* dataname)
{
  TFile* fout= TFile::Open (outfile, "recreate");
  if (!fout) return 1;
  INFO("Copy%s workspace '%s' to file %s", ((edit && *edit) ? " and edit" : ""), w->GetName(), fout->GetName());
  int nerr= editws (w, fout, edit, mcname, dataname);
  delete fout;
  return nerr;
}

int editws (TDirectory* f, TDirectory* fout, const char* edit, const char* wsname, const char* mcname, const char* dataname)
{
  INFO("Copy%s workspaces from file %s to file %s", ((edit && *edit) ? " and edit" : ""), f->GetName(), fout->GetName());
  int nws=0, nerr=0;
  if (wsname && wsname[0]) {
    RooWorkspace* w= 0;
    ...
    while ((key= dynamic_cast<TKey*>(nextKey()))) {
      ...
      if (c && c->InheritsFrom (RooWorkspace::Class())) {
        const TKey* ktop= dynamic_cast<TKey*>(f->GetListOfKeys()->FindObject(key->GetName()));
        if (ktop && key->GetCycle() != ktop->GetCycle()) continue;  // check this is the highest cycle
  ...
  if (!nws) { WARN("No workspaces found in file %s",f->GetName()); nerr++; }
  return nerr;
}

int editws (TDirectory* f, const char* outfile, const char* edit, const char* wsname, const char* mcname, const char* dataname)
{
  TFile* fout= TFile::Open (outfile, "recreate");
  if (!fout) return 1;
...
int editws (const char* outfile, const char* edit, const char* wsfile, const char* wsname, const char* mcname, const char* dataname)
{
  TFile *f, *fopened=0;
  if (wsfile && wsfile[0]) {
    ...
    if (!f) ERROR("No input workspace file specified");
  }
  if (!f) return 1;
