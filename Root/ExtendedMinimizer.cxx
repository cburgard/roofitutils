...
RooLinkedList* makeList( const RooCmdArg& arg1, const RooCmdArg& arg2, const RooCmdArg& arg3, const RooCmdArg& arg4,
  ...
  void setVals(const RooAbsCollection& vars, const RooAbsCollection* snap, bool setConstant = false){
    if(!snap) return;
    RooAbsArg* obj;
  ...
  fMinimizerAlgo( "Migrad" )
{
  ...
  coutP(InputArguments) << "ExtendedMinimizer::ExtendedMinimizer(" << fName <<") created" << std::endl;
}
...
int ExtendedMinimizer::minimize( const RooCmdArg& arg1, const RooCmdArg& arg2, const RooCmdArg& arg3, const RooCmdArg& arg4,
                                 ...
                                 const RooCmdArg& arg9, const RooCmdArg& arg10, const RooCmdArg& arg11, const RooCmdArg& arg12 )
{
  RooLinkedList* l = makeList(arg1,arg2,arg3,arg4,arg5,arg6,arg7,arg8,arg9,arg10,arg11,arg12);
...
void ExtendedMinimizer::setup()
{
  ...
  if (!fReuseNLL || !fNll) {
    if (fReuseNLL) {
      coutW(InputArguments) << "ExtendedMinimizer::createMinimizer: NLL is supposed to be re-used but not present, creating new NLL!"<<std::endl;
    }
    coutI(InputArguments) << "Creating new Nll"<<std::endl;
    if (fNll){
      coutW(InputArguments) << "ExtendedMinimizer::createMinimizer: deleting previous NLL!"<<std::endl;
      delete fNll;
    }
	...
	if(n>0) coutP(InputArguments) << "Fixed cache of " << n << " instances of RooStarMomentMorph" << std::endl;
      }
    }
  ...
  if(fWorkspace){
    if(RooFitUtils::RooStarMomentMorphFix){
      int n = fixRooStarMomentMorph(fWorkspace);
      if(n>0) coutP(InputArguments) << "Re-Fixed cache of " << n << " instances of RooStarMomentMorph" << std::endl;
    }
  }
  
  if (!fNll) {
    throw std::runtime_error("[Error] ExtendedMinimizer::setup: Failed to obtain NLL");
  }
...
template <class A> int ExtendedMinimizer::parseConfig( const A& cmdList )
{
  ...
  inverseFilterCmdList(fNllCmdList, "NumCPU,Constrained,Constrain,CloneData,GlobalObservables,GlobalObservablesTag,OffsetLikelihood");
...
ExtendedMinimizer::Result::Eigen* ExtendedMinimizer::eigenAnalysis(const TMatrixDSym& hesse){
  ...
  ExtendedMinimizer::Result::Eigen* result = new ExtendedMinimizer::Result::Eigen(
										  Geigen.GetEigenValues(),
...
ExtendedMinimizer::Result::Minimization ExtendedMinimizer::robustMinimize(){
  ...
  while (true){
    ...
    coutP(ObjectHandling) << "ExtendedMinimizer::robustMinimize(" << fName << "): starting minimization with strategy " << strategy << std::endl;
    status =  fMinimizer->minimize(fMinimizerType.c_str(), fMinimizerAlgo.c_str());
    const double nllval = fNll->getVal();
    
    if((isnan(nllval) || isinf(nllval) || (status != 0 && status != 1)) && strategy < 2 && retry > 0){
      coutW(ObjectHandling) << "ExtendedMinimizer::robustMinimize(" << fName << ") fit failed with status " << status << ". Retrying with strategy " << strategy << std::endl;
      strategy++;
      retry--;
    } else {
      break;
  ...
  if (!mini.ok()) {
    coutE(ObjectHandling) << "ExtendedMinimizer::robustMinimize(" << fName << ") fit failed with status " << status << std::endl;
  } else {
    coutP(ObjectHandling) << "ExtendedMinimizer::robustMinimize(" << fName << ") fit completed with status " << status << std::endl;
  }
  coutP(ObjectHandling) << "ExtendedMinimizer::minimize(" << fName << "): Evaluating Nll" << std::endl;
...
ExtendedMinimizer::Result* ExtendedMinimizer::run(){
  ...
  if (fHesse) {
    ...
    if(covqual == 0){
      coutP(ObjectHandling) << "ExtendedMinimizer::minimize(" << fName << "): Covariance quality is " << covqual << ". Running Hesse... " << std::endl;
      fMinimizer->hesse();
    }
    ...
    Double_t determ = 0;
    coutP(ObjectHandling) << "ExtendedMinimizer::minimize(" << fName << "): attempting to invert covariance matrix... " << std::endl;
    TMatrixDSym origG = fMinimizer->lastMinuitFit()->covarianceMatrix();
    TMatrixDSym G = origG.Invert(&determ);
    if(determ == 0 || isnan(determ)){
      coutE(ObjectHandling) << "ExtendedMinimizer::minimize(" << fName << "): covariance matrix is singular! " << std::endl;
    } else {
      r->hesse = new TMatrixDSym(G);
      // Eigenvalue and eigenvector analysis
      if (fEigen) {
        coutP(ObjectHandling) << "ExtendedMinimizer::minimize(" << fName << "): starting eigen analysis... " << std::endl;
  ...
  if(r->min.status >= 0){
    ...
    coutP(ObjectHandling) << "ExtendedMinimizer::minimize(" << fName << "): Running Minos" << std::endl;
    if (fMinos) {
      if (fMinosSet) {
  ...
  if (fSave) {
    ...
    std::string title = Form("Result of fit of p.d.f. %s to dataset %s",GetName(),fData->GetName());
    coutP(ObjectHandling) << "ExtendedMinimizer::minimize(" << fName << ") saving results as " << name << std::endl;
    r->fit = fMinimizer->save(name.c_str(),title.c_str());
    if(!r->hesse) r->fit->setCovQual(-1);
  }
...
void ExtendedMinimizer::scan(Result* r, const std::vector<std::string>& parnames, const std::vector<std::vector<double> >& points){
  ...
  for(auto pname:parnames){
    RooRealVar* v = dynamic_cast<RooRealVar*>(attachedSet->find(pname.c_str()));
    if(!v){
      throw std::runtime_error(TString::Format("unknown parameter name: %s",pname.c_str()).Data());
    }
    params.push_back(v);
  }
...
void ExtendedMinimizer::findSigma(Result* r,const RooAbsCollection& scanSet){
  ...
  for (RooLinkedListIter it = scanSet.iterator(); RooRealVar* v = dynamic_cast<RooRealVar*>(it.Next());) {
    if(!v){
      throw std::runtime_error("invalid variable!");
    }
    coutI(ObjectHandling) << "ExtendedMinimizer::findSigma(): starting scan of " << v->GetName() << std::endl;
    ...
    coutI(ObjectHandling) << "ExtendedMinimizer::minimize(" << fName << ") " << std::endl;
    delete snap;
  }
}
...
Double_t ExtendedMinimizer::findSigma(ExtendedMinimizer::Result* result, const double guessval, const double val_mle, RooRealVar* par, const double nsigma,const int maxiter){
  ...
  for (; iter < maxiter; iter++) {
    coutI(ObjectHandling) << "ExtendedMinimizer::findSigma(" << fName << ") " << Form("Parameter %s %+gsigma iteration %d: start %g (MLE%+g)", par->GetName(), nsigma, iter+1, val_guess, val_guess-val_mle) << std::endl;
    ...
    for (ExtendedMinimizer::ValueMap::iterator iguess= guess_to_corr.begin(); iguess != guess_to_corr.end(); ++iguess) {
      if (fabs(iguess->first - val_pre) < direction*val_pre*0.02) {
        damping_factor *= 0.8;
        coutW(ObjectHandling) << "ExtendedMinimizer::findSigma(" << fName << ") Changing damping factor to " << damping_factor << std::endl;
        if (nDamping++ > 10) {
          nDamping = 1;
          damping_factor = 1.0;
        }
    ...
    coutI(ObjectHandling) << "ExtendedMinimizer::findSigma(" << fName << ") " << Form("%s %.3f (MLE%+.3f) -> %.3f (MLE%+.3f), change %+.3f, precision %.3f, -2lnL %.4f, sigma(guess) %.3f", par->GetName(), val_pre, val_pre-val_mle, val_guess, val_guess-val_mle, delta, relprecision, tmu, sigma_guess) << std::endl;
    
    coutI(ObjectHandling) << "ExtendedMinimizer::findSigma(" << fName << ") NLL:                 " << nll << std::endl;
    coutI(ObjectHandling) << "ExtendedMinimizer::findSigma(" << fName << ") delta(NLL):          " << nll-nllmin << std::endl;
    coutI(ObjectHandling) << "ExtendedMinimizer::findSigma(" << fName << ") nsigma*sigma(pre):   " << fabs(val_pre-val_mle) << std::endl;
    coutI(ObjectHandling) << "ExtendedMinimizer::findSigma(" << fName << ") sigma(guess):        " << sigma_guess << std::endl;
    coutI(ObjectHandling) << "ExtendedMinimizer::findSigma(" << fName << ") par(guess):          " << val_guess+corr << std::endl;
    coutI(ObjectHandling) << "ExtendedMinimizer::findSigma(" << fName << ") best-fit val:        " << val_mle << std::endl;
    coutI(ObjectHandling) << "ExtendedMinimizer::findSigma(" << fName << ") tmu:                 " << tmu << std::endl;
    coutI(ObjectHandling) << "ExtendedMinimizer::findSigma(" << fName << ") Precision:           " << direction*val_guess*precision << std::endl;
    coutI(ObjectHandling) << "ExtendedMinimizer::findSigma(" << fName << ") Correction:          " << -corr << std::endl;
    coutI(ObjectHandling) << "ExtendedMinimizer::findSigma(" << fName << ") nsigma*sigma(guess): " << fabs(val_guess-val_mle) << std::endl;
  ...
  if (!ok) {
    std::cerr << "findSigma failed after " << iter << " iterations" << std::endl;
  ...
  coutI(ObjectHandling) << "ExtendedMinimizer::findSigma(" << fName << ") " << Form("%s %+gsigma = %.3f at -2lnL = %.4f after %d iterations", par->GetName(), nsigma, err, tmu, iter+1) << std::endl;
...
ExtendedMinimizer::GraphPair ExtendedMinimizer::prepareProfile(ExtendedMinimizer::ValueMap map_poi2nll) {
   ...
   for (ExtendedMinimizer::ValueMap::iterator it_poi = map_poi2nll.begin(); it_poi != map_poi2nll.end(); ++it_poi) {
